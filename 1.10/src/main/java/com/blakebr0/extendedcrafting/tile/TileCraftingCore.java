...
import com.blakebr0.extendedcrafting.lib.PedestalPosition;
import com.blakebr0.extendedcrafting.tile.TilePedestal.StackHandler;
...
public class TileCraftingCore extends TileEntity implements ITickable {
	...
	private final CustomEnergyStorage energy = new CustomEnergyStorage(10000000);
	
	private static ArrayList<PedestalPosition> pedestalPositions;
	public CombinationRecipe activeRecipe;
	private int progress;
	private int oldEnergy;
	
	static {
		PedestalPosition.Pedestals.PEDESTALS.setupPositions();
		pedestalPositions = PedestalPosition.Pedestals.PEDESTALS.pedestalPositions;
	}
	...
	private static List<CombinationRecipe> getValidRecipes(ItemStack stack){
		...
		if(!StackHelper.isNull(stack)){
			for(CombinationRecipe recipe : CombinationRecipeManager.getRecipes()){
	...
	private List<TilePedestal> getPedestalsWithStuff(CombinationRecipe recipe){
		...
		for(int i = 0; i < this.pedestalPositions.size(); i++){
			BlockPos pos = this.getPos().add(this.pedestalPositions.get(i).getPos());
