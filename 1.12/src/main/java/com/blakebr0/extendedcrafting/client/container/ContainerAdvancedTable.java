...
public class ContainerAdvancedTable extends Container {
	...
	public ContainerAdvancedTable(InventoryPlayer player, TileAdvancedCraftingTable tile, World world) {
		...
		this.addSlotToContainer(new TableCraftResultHandler(player.player, this.matrix, this.result, 0, 142, 50));
		int wy;
		int ex;

		for (wy = 0; wy < 5; ++wy) {
			for (ex = 0; ex < 5; ++ex) {
				this.addSlotToContainer(new SlotItemHandler(handler, ex + wy * 5, 14 + ex * 18, 15 + wy * 18));
			}
		}
