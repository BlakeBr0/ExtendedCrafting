...
import com.blakebr0.extendedcrafting.tileentity.EliteAutoTableTileEntity;
...
public class EliteAutoTableBlock extends BaseTileEntityBlock {
    ...
    public TileEntity createTileEntity(BlockState state, IBlockReader world) {
        return new EliteAutoTableTileEntity();
    }
    ...
    public ActionResultType onBlockActivated(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockRayTraceResult trace) {
        if (!world.isRemote()) {
            ...
            if (tile instanceof EliteAutoTableTileEntity)
                player.openContainer((EliteAutoTableTileEntity) tile);
        }
    ...
    public void onReplaced(BlockState state, World world, BlockPos pos, BlockState newState, boolean isMoving) {
        if (state.getBlock() != newState.getBlock()) {
            TileEntity tile = world.getTileEntity(pos);
            if (tile instanceof EliteAutoTableTileEntity) {
                EliteAutoTableTileEntity table = (EliteAutoTableTileEntity) tile;
