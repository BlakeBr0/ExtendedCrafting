...
public class AdvancedAutoTableContainer extends Container {
	...
	private AdvancedAutoTableContainer(ContainerType<?> type, int id, PlayerInventory playerInventory, PacketBuffer buffer) {
		this(type, id, playerInventory, p -> false, new ItemStackHandler(26), new IntArray(5), buffer.readBlockPos());
	}

	private AdvancedAutoTableContainer(ContainerType<?> type, int id, PlayerInventory playerInventory, Function<PlayerEntity, Boolean> isUsableByPlayer, IItemHandlerModifiable inventory, IIntArray data, BlockPos pos) {
		super(type, id);
	...
	public ItemStack transferStackInSlot(PlayerEntity player, int slotNumber) {
		...
		if (slot != null && slot.getHasStack()) {
			...
			} else if (!this.mergeItemStack(itemstack1, 27, 63, false)) {
				return ItemStack.EMPTY;
			}
	...
	public static AdvancedAutoTableContainer create(int windowId, PlayerInventory playerInventory, Function<PlayerEntity, Boolean> isUsableByPlayer, IItemHandlerModifiable inventory, IIntArray data, BlockPos pos) {
